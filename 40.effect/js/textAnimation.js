function textAni(cssSelector,option){
    ...
    let lettersLenght = $(cssSelector).get(0).innerText.length
    let result = ""
    for(let i = 0; i<lettersLenght; i++){
    ...
    // 기본값 적용 -> 삼항연사자로 표현
        ...
        let aniTextSize = option.textSize?option.textSize:20
        let aniName = option.effect?option.effect:"introAni"
        // aniName변수에는 숫자값으로 변경되는 효과가 아닌 "이름"으로 변경되는 효과를 저장
        
        
    
        $(cssSelector).css("font-size","0")
        $(cssSelector +" b").css("opacity","0").css("line-height","1").css("display","inline-block").css("font-size",aniTextSize+"px")
